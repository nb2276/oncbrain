 ...
 */
// Cheap (name, mtime, size) signature so the strict reader can skip re-parsing
// an unchanged corpus on every per-page call (the build-time O(pages × files)
// cliff) while auto-invalidating when any file changes. Keyed by dir so the
// per-dir callers (slug-uniqueness, tag-foundations, tests) don't share a slot.
function strictDirSignature(dir: string): string {
  const parts: string[] = [];
...
let _listDigestsStrictCache: { key: string; value: DigestArtifact[] } | null = null;

// Reset the listDigestsStrict memo (tests that rewrite a digests dir in-place).
export function resetListDigestsStrictCache(): void {
  _listDigestsStrictCache = null;
}

export function listDigestsStrict(digestsDir: string = DIGEST_ROOT): DigestArtifact[] {
  if (!existsSync(digestsDir)) return [];
  // Collision-proof cache key without a raw delimiter: JSON-encode the
  // [dir, signature] pair so two different pairs can never serialize to the
  // same string. A literal separator is unsafe (a path can contain a space,
  // colon, or |, all of which appear in dirs/signatures); the previous NUL
  // delimiter was collision-proof but made the file read as binary to grep.
  const key = JSON.stringify([digestsDir, strictDirSignature(digestsDir)]);
  if (_listDigestsStrictCache && _listDigestsStrictCache.key === key) {
    return _listDigestsStrictCache.value;
  }
  ...
  // Cache only a fully-validated read (a malformed file throws above and is
  // never cached, so a later fix is picked up).
  _listDigestsStrictCache = { key, value };
  return value;
}
