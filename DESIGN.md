...
8. **"supersedes the {date} read" line (v0.53).** A hairline muted line, same treatment as the v0.37 "updated from" line, shown when this card replaces an earlier reading of the same trial's same objective — a conference abstract that has become the full publication. Distinct from "updated from", which reports that a NUMBER moved; this reports that a whole earlier CARD is being retired in favour of this one.
   - **It links the predecessor only while the predecessor is still published.** Once lineage has queued the earlier card for removal the link is unsafe in both directions: it may already be gone, or the queued rebuild may not have run yet. The site serves `catchall_document: index.html`, so a dead study URL returns **200 with the home page** rather than 404ing — a silent wrong page is worse than no link. Both states resolve to "name the date, don't link it".
   - The copy says **"queued for removal"**, never "unpublished". Writing the override and enqueueing the rebuild is all the build did; the predecessor comes down when that rebuild runs, and a failed rebuild does not block the publish. The card must not claim more than the build can guarantee.
...
### Withheld prose (v0.54)

A card can be missing a section or a bullet by design. The comparator grounding gate withholds any sentence that attaches a figure to a trial the card's own sources never mention, at the finest unit containing the claim — the offending bullet, not the bullet list; the offending section, not the fold.

Visually this is a **non-event**: there is no placeholder, no "withheld" marker, no gap treatment. A reader should see a shorter card, not a damaged one. Do not add an affordance here — the absence is the correct rendering, and marking it would advertise a failure the reader cannot act on. The build log and `formatGroundingWithholds()` are where that belongs.
