...
- **ONE ruler per (endpoint class, ratio kind), computed across the whole corpus** (`effectDomains()` in `digest-data.ts`, memoized per build). An odds ratio and a hazard ratio never share a ruler.
