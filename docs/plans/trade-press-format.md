...
### M2 RESULT (2026-06-13) — DONE; 1301 tests pass, 0 type errors, render-verified on a synthetic build
- **Provenance** (`StudyCard.astro`): a "🗞️ Reported via {outlet} →" line under the TL;DR, gated on TRADE-PRESS-ONLY (no tweets/slides, every paper trade-press via `tradePressLabel()`, single outlet) — suppressed on mixed clusters (Codex #5). Independent of content_type, so a single-study trade report gets it too. The href reuses the http(s)-only guard. Outlet name is the authoritative URL-host label, not the free-text journal.
- **Discussed-trials** (`StudyCard.astro`): for a `review`, a "Trials discussed" callout (same shell as the comparator block) rendering `discussed_trials` as dot-separated PLAIN TEXT — no links (no NCT inference).
- **Rail glyph** (Codex #10, DRY): the per-page `verdictMetaFor(...)?.emoji ?? '·'` in all three rail builders is replaced by one shared `railEmojiForStudy(study)` in `verdict.ts` — verdict emoji if present, **🗞️ for a verdict-less review** (reads as a press round-up, not a study awaiting triage), neutral `·` otherwise. (The prior `·` fallback already existed identically in all three, so the "blank rail" risk Codex flagged was already absent; this upgrades it to a distinct review glyph.)
- **Verdict suppression**: confirmed at render — the review card emits NO verdict pill, the single-study trade report KEEPS its pill (the M1 build-time strip + the existing `study.verdict?` guards do the work; no card change needed).
- **Verified** by an `astro build` against a throwaway synthetic future-date digest (review + single-study trade report); grepped the built HTML for both provenance lines, the four acronyms, the 🗞️ rail glyph, and verdict-pill count = 1. Fixture removed; clean rebuild restored the real latest digest (the pwa-build precache test is dist-dependent — rebuild after any scratch build).
- **Tests**: `test/verdict.test.ts` (`railEmojiForStudy` across verdict / review / plain-no-verdict).
- **Not touched**: Obsidian twin + JSON API don't yet surface the provenance line / discussed-trials list (they carry the raw fields via the artifact, but render study-report-style). Acceptable for the public-web milestone; a follow-up if the curator wants the acronym list in the vault note.
