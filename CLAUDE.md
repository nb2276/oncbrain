...
- **Tests**: Vitest (1959 tests across 95 files as of v0.39). `test/global-setup.ts` builds `dist/` once before collection when the artifacts are absent, so `npm test` is correct cold or warm.
...
# Tests + eval
npm test                        # vitest run (1959 tests)
...
```
npm test                   # 1959 tests across 95 files, all should pass
