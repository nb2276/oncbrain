...
- **Tests**: Vitest (1989 tests across 97 files as of v0.40). `test/global-setup.ts` builds `dist/` once before collection when the artifacts are absent, so `npm test` is correct cold or warm.
...
# Tests + eval
npm test                        # vitest run (1989 tests)
...
```
npm test                   # 1989 tests across 97 files, all should pass
...
**Released:** v0.40.0 (channel post + date card de-duplicated: the OG card owns `top_line`, the Telegram body owns inventory + link, and a 1-study day collapses to site + verdict. Date card headline is two-tier via `splitHeadline` — splits the curator's sentence at its OWN punctuation (paren-depth-0, lead >=40%), never rewrites; unmarked cards centre vertically. Prior: v0.39.2 (content-level publish boundary: test/publish-boundary-content.test.ts reads the local DB and asserts no sentence of fulltext_excerpt_md / figure_ocr_md / figure_structured_md appears in dist/ unless it is also title, abstract, authors_json or the curator's own bookmark slide OCR. The structural test guards field NAMES; this guards the words, because Phase 2 is handed the full text as grounding and could quote it. Skips without a local DB). Prior: v0.39.1 (notify-CLI DRY: build/notify-curator.ts + build/notify-channel.ts share src/lib/notify-cli.ts for arg parsing, PUBLIC_SITE_URL, digest load and the script-guard + fail-soft wrapper. Closes the drift where v0.39 guarded the curator DM and left notify-channel running main() on import, which posts to the PUBLIC channel. Deploy-readiness POLICY stays per-CLI on purpose: the DM sends on timeout with the preview suppressed, the channel skips). Prior: v0.39.0 (stranded-publish fix: daily-build.sh unions dates from commits ahead of origin/main into CHANGED_DATES so a date a failed push stranded is announced by the run that finally pushes it, pushes a backlog even with nothing new to commit, checks `git commit` before announcing, and pushes an explicit refspec. Date logic lives in scripts/lib/publish-dates.sh, tested against real git repos). Prior: v0.38.0 (E6 reader compare tray: select 2+ studies sharing an endpoint bucket + ratio kind and read their marks stacked on one axis. CLONES the rendered mark SVG rather than recomputing geometry; the first pick LOCKS the set and incompatible controls are disabled, not dimmed; no control at all where a page has no comparable pair). Prior: v0.37.0 (E5 longitudinal magnitude: a trial that comes back with a DIFFERENT effect size for the same endpoint carries an `updated from` line + a hollow prior dot, and the build/curator DM name the move. Strict matching — a disagreement on NCT or acronym vetoes the match; an unchanged estimate never fires). Prior: v0.36.0 (effect-size marks slice 4: the mark travels onto the per-study share card, painted in satori from the same `markGeometry()` the web SVG uses, with a per-study try/catch so one study's mark can never fail the nightly publish). Prior: v0.35.0 (effect-size marks slice 3: ONE corpus-wide ruler per endpoint FAMILY + ratio kind, so a study renders identically on every surface and site-page marks are mutually comparable). Prior: v0.34.0 (slice 2: paired bars for two-value endpoints, plus a POSITIVE gate that only reads a detail table when it can be identified as endpoint-by-arm). Prior: v0.33.0 (the ratio forest dot on a log axis, with a shared scale per date). Prior: v0.32.0 — per-specialty "why it matters" (`significance_by_specialty`): the specialty bar reframes the callout prose to the reader's field instead of only dimming cards, plus specialty-filter discoverability, search on the header's title row, and the deploy-readiness gate on Telegram notifications. Builds on v0.31 (reader-selectable specialty relevance) and v0.30 (the "endpoint-forward card" — `primary_endpoint` / `analysis_sections` — plus the dark-default theme).
