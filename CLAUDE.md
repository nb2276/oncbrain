...
admin form ───┘                               │
                              ...
                              build:day ─▶ 3-phase LLM (group → study agent →
                                ...
                                → comparator grounding gate (withhold a number
                                  attached to an unsourced trial)
                                → trial lineage (update / new-card / duplicate;
                                  auto-suppress DEFAULT OFF)
                                              │
...
npm run backfill:facets                                                  # v0.53: classify report_facet/maturity on sources ingested before lineage. Metadata only — never suppresses. --id / --date / --slug / --dry-run / --force
...
DigestStudy {
  ...
  primary_endpoint?: {name, klass, stat_value, stat_detail} | null  // v0.30: the headline endpoint the "endpoint-forward card" leads with. `klass` is the endpoint class (overall-survival | local-control | surrogate | safety) → a legibility chip (dropped for overall-survival). `stat_value` is the effect size verbatim (e.g. "HR 0.53"), `stat_detail` the CI/detail. Validated grounded (validatePrimaryEndpoint); null when the source has no clean headline stat
  analysis_sections?: {label, body}[]  // v0.30: labeled prose depth sections rendered in the card fold (Design · Population & inclusion · Regimen · Radiotherapy · Endpoints · Results · Safety · vs leading data · Applies to · Limitations · Discussion). Each `body` is 1-3 grounded sentences; a Limitations section must ADD to the verdict rationale, not echo it
  relevant_specialties?: SpecialtyTag[] | null  // v0.31: which subspecialties this study informs (radonc | medonc | surgonc), judged NEUTRALLY (not by modality), ≥1 when set. Renders `data-specialties` on the card → the reader-selectable SpecialtyBar dims off-target cards client-side. Null/absent when unjudged
  significance_by_specialty?: Partial<Record<SpecialtyTag, string>> | null  // v0.32: per-specialty "why it matters" — one grounded 2-3 sentence read per relevant specialty, from that reader's decision lens (radonc: dose/target/local control; medonc: regimen/biomarker; surgonc: resectability/margin). StudyCard renders the default `significance` visible + a hidden variant block per key; the SpecialtyBar swaps to the reader's picked specialty's read. Null for a single-specialty study or when nothing specialty-specific is additive
  ...
  supersedes?: {date, slug, stat_value, stat_detail, auto_dropped, declined_reason, droppable} | null
                           // v0.53: the EARLIER card this one supersedes (same trial, same objective, matured). Build-stamped by lineage-pass.ts, never LLM-generated. `auto_dropped` means the removal was QUEUED, not completed — the card links the predecessor only while it is still published, because catchall_document:index.html turns a dead study URL into a 200 home page rather than a 404
  doi?: string | null      // v0.55: the study's own publication DOI. Parallel to `nct`: Phase 2 may emit it and a regex backstop (soleDoiIn — exactly one in the sources, or nothing) fills it in. Stored VERBATIM, not normalized: normalizeDoi lowercases, which is right for identity and wrong for a quoted citation
...
src/
  lib/
    ...
    trial-lineage.ts       v0.53: the DECISION layer for a trial that comes back — update / new-card / duplicate / unrelated. suppressionBlockers() is the single authorization gate for anything destructive and reports EVERY failing precondition, not the first (mislabelling one blocker as another let the curator DM offer a drop the gate had refused). Owns sameTrialIdentity(), the ONE definition of trial identity, which prior-estimate.ts also calls
    lineage-pass.ts        v0.53: the BUILD-TIME half — reads facets/identity out of the source DB, plans the action, and enforces what the decision layer cannot see: never empty a date (over the whole suppression set plus that date's existing override), never act on a card whose own objective is ambiguous, never act on a card built from >1 substantive source. gateAuthorized (evidence) and autoSuppress (policy) are deliberately separate
    source-facet.ts        v0.53: the LLM half of lineage — what a source REPORTS about its trial (report_facet / maturity / followup_months / its own acronyms), a closed enum written at enrich time. Off-enum parses to null; abstention is a correct answer for a guideline or review
    comparator-grounding.ts v0.54/v0.55: fidelity to source, both directions. withholdUngroundedComparators() drops prose attaching a NUMBER to a trial the sources never mention (scope measured: requiring every named trial to be sourced withheld 105/124 cards); droppedDiseaseStates() WARNS when a card loses a disease-state qualifier its sources state; conflictingState() blocks a related-trials pick whose clinical state contradicts the study's — axis-and-synonym based, and hormone sensitivity is deliberately NOT an axis so sibling watches survive
    rebuild-window.ts      v0.53: queuedAfterBuild() — is a queue entry NEWER than the build that supposedly satisfied it. Lives in its own side-effect-free module because build/rebuild-queued.ts calls main() at module scope, and importing that opened the real DB and could spawn paid builds
    ...
    prior-estimate.ts      v0.37 (E5): longitudinal magnitude. Build-time (NOT enrich-time: primary_endpoint only exists after Phase 2) detection of the same trial's PREVIOUS reading of the same endpoint, when it moved. Strict identity — NCT or acronym match, with BOTH conflict guards, so a trial that misprints its registration (RADIOSA publishes ORIOLE's NCT) can never borrow another's estimate. Abstains on an unchanged number, a different endpoint bucket, and anything ambiguous. Feeds the card's `updated from` line, the hollow prior dot, and the curator DM
  ...
  components/CompareTray.astro v0.38 (E6): the reader compare tray. Mounted on [date], sites/[site], tags/[...slug] and studies. Clones each selected card's rendered `svg.emark` (never recomputes geometry), locks the set to the first pick's `data-compare-key` (endpointFamily::ratioKind|domainLo:domainHi) and DISABLES incompatible controls, prunes a card the tag rail hides, and renders nothing where a page has no comparable pair. In-memory, same-page, no storage
...
scripts/
  ...
  lib/publish-dates.sh     v0.39: which dates a publish will carry — this run's STAGED digests unioned with the NET diff of commits ahead of origin/main (a failed push strands a commit locally; the next run's push deploys it, so that run has to announce it). Sourceable, so test/publish-dates.test.ts exercises it against real git repos under the same `set -uo pipefail` the cron uses
...
- Don't attach a figure to a trial the sources don't mention. Naming a prior trial as context is the point of `vs leading data`; asserting its hazard ratio from memory is fabrication, and a build-time gate withholds the sentence (`comparator-grounding.ts`).
- Don't add a guard to one surface and assume the class is closed. Four separate bugs in v0.53-v0.55 were "secured the mechanism, left the caller open": a drop token offered past the gate it was meant to respect, a population rule bound to one field and then to one phase, a grounding gate that ran in production but never in the eval. Ask which OTHER path reaches the same act.
- Don't reason a rule into place without measuring it against `data/digests/`. Three rules were wrong on first writing and the corpus said so: a comparator gate that would have withheld 105 of 124 cards, a state check that treated mHSPC and mCSPC as opposites when they are synonyms, a token match that read "nmCRPC" as also naming "mCRPC".
