...
- **Review consistency on secondary surfaces (v0.16 ship review — red-team).** **Priority: P3.** A `content_type:review` reads as a 🗞️ press round-up on the date / site / tag pages (shared `railEmojiForStudy`) but two other study surfaces don't yet distinguish it: (1) the home `RecentFeed` shows the disease-site emoji (it deliberately flags only practice-changing with 🚀 — decide whether a review warrants its own marker there); (2) the Obsidian vault twin (`renderObsidian`) omits the `discussed_trials` list + review framing, so a review note is a bare verdict-less study. Both are **design/scope judgments, not bugs** — the web milestone is intentionally first. Decide per-surface whether to mirror the review treatment or document it as web-only.
