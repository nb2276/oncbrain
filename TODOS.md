...
## Grounding (surfaced by the v0.53 eval gate)

- **Phase 2 fabricates comparator trials with specific effect sizes.**
  **Priority: P0.** `npm run eval` scores 5.0 against a threshold of 8, and the
  judge's hallucination findings are the cap: the digest cites "VISION (NEJM
  2021): Lu-PSMA-617 monotherapy rPFS HR 0.40 vs SOC in post-taxane mCRPC" and
  "DESTINY-Breast04 (post-chemo HER2-low)" as grounded comparators in its
  "vs leading data" sections — neither trial, publication, nor effect size
  appears anywhere in the input sources. One run also flipped EMBARK's stated
  population from nmCRPC to castration-sensitive, a clinically opposite
  hormone-sensitivity status.

  NOT introduced by v0.53: a control run with the v0.53 grouping-prompt change
  reverted scored the same 4.0 factual accuracy, so this is present on main and
  predates the release. Confirmed by two independent eval runs.

  This is the failure mode the whole product premise rules out — every number in
  a digest is supposed to be traceable to a source. The comparative-context
  sections are where it leaks, because they invite the model to reach for
  literature that is not in front of it. Worth its own release: likely a
  grounding gate over "vs leading data" prose (the figure pipeline already has
  one in figure-extract.ts), or removing the invitation from the Phase 2 prompt.
  (v0.53 eval + control run, 2026-08-15)
