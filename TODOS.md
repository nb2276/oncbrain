...
- **T4 — shareable card-as-image to X.** Build-time render of a study card (verdict + headline number + @handle) to `public/share/<date>/<slug>.png` via satori + resvg; doubles as the per-study `og:image`. Share button gains "Share image" (`navigator.share({files})` where supported, else link). Hard constraint: stays inside the publish-boundary (synthesized text + verdict only, never PDF/slide pixels — guard in `test/publish-boundary.test.ts`). (office-hours design T4)
