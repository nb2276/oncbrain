...
## Review-trial ingestion (v0.17) follow-ups

- **Cross-date review-trial surfacing (codex review #4).** **Priority: P3.** `papers.pmid` is UNIQUE, so one paper row = one date. When a 2nd review (on a later date) names a trial whose primary paper is already a source on an earlier date, approving it can't surface a card on the 2nd date — the acronym stays plain text there (the trial's card lives on its first date). `ingestApprovedResolutions` logs this loudly + skips. The proper fix is a **manifest-as-date-association**: include a date's approved-resolution PMIDs in that date's `papersForDate` build inputs (the manifest, not the papers table, carries the many-to-many review-date↔paper link), handling the bookmark_date-mismatch + provenance-pill edges. **Context:** `src/lib/review-trial-ingest.ts` (the cross-date branch), `build/digest-builder.ts` (papersForDate), `docs/plans/review-trial-ingestion.md`.
- **Studio menu entry for review-trial approval.** **Priority: P3.** T4 shipped the approval flow as the standalone `resolve:review-trials --review` @clack CLI; a `studio.ts` menu item (discoverability) was deferred (it needs the CLI's `main()` de-coupled into callable functions). Wire a "Resolve review trials" entry that calls the resolve + approval functions. **Context:** `build/studio.ts`, `build/resolve-review-trials.ts`.
