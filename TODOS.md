...
- ~~**Conference badge for paper/slide-only days.**~~ **DONE (pending release).** `dominantConferenceForDate` (src/lib/db.ts:605) now unions bookmarks + papers + slides (unanimous-single-slug semantics preserved), so a paper/PDF/slide-only conference day gets a badge + `/conferences/<slug>` page. Untagged sources still ignored. Tests in `test/db.test.ts`. (follow-up from v0.14.9 conference auto-detect review)
