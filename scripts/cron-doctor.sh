#!/bin/bash
# cron-doctor: surface why the 1am cron didn't run.
#
...
#   3. Mac asleep at 1am with no pmset wake schedule
...
if [ -z "$LAUNCH_LINE" ]; then
  ...
  case "$STATUS" in
    ...
    *)   warn "non-zero last exit ($STATUS). Inspect log near the last 1am window." ;;
  esac
fi
...
if echo "$SCHED" | grep -qiE "wake|poweron"; then
  ...
  fail "NO wake schedule. If Mac sleeps overnight, launchd will NOT fire 1am cron."
  info "fix: sudo pmset repeat wakeorpoweron MTWRFSU 00:55:00"
fi

# ── 4. Was the Mac asleep at the last 1am window? ──────────────────────
echo ""
...
if [ -f "$LOGFILE" ]; then
  ...
  if [ -z "$HEADERS" ]; then
    ...
    echo "$HEADERS" | sed 's/^/     /'
    echo ""
    NIGHT_RUNS=$(echo "$HEADERS" | grep -E "0[0-2]:[0-5][0-9]:[0-9]{2}" | wc -l | tr -d ' ')
    if [ "$NIGHT_RUNS" -gt 0 ]; then
      pass "$NIGHT_RUNS run(s) in the 00:00-02:59 window (real cron firings)"
    else
      warn "no run timestamps fall in the 12-3am window. Every recorded run was likely manual."
    fi
  fi
else
...
if ! pmset -g sched 2>&1 | grep -qiE "wake|poweron"; then
  ACTIONS+=("sudo pmset repeat wakeorpoweron MTWRFSU 00:55:00   # wake the Mac 5min before the 1am fire")
fi
